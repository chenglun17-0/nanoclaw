...
This isn't a framework or a platform. It's working software for my specific needs. I use Feishu, so it supports Feishu. I don't use Telegram, so it doesn't support Telegram. I add the integrations I actually want, not every possible integration.
...
A personal Claude assistant accessible via Feishu, with minimal custom code.
...
### Message Routing
- A router listens to Feishu and routes messages based on configuration
...
- **Global memory**: `groups/global/CLAUDE.md` provides shared context for non-main groups
- **Files**: Groups can create/read files in their folder and reference them
- Agent runs in the group's folder and receives group-specific memory plus optional shared global context
...
- Can manage shared context in `groups/global/CLAUDE.md`
...
### Feishu
- Using Feishu Open Platform SDK (`@larksuiteoapi/node-sdk`) for event subscription and messaging
- Messages stored in SQLite, polled by router
- App credentials configured via `.env` (`FEISHU_APP_ID`, `FEISHU_APP_SECRET`)
...
### Skills
- `/setup` - Install dependencies, configure Feishu credentials, configure scheduler, start services
